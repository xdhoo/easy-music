...
export class AudioService {
  ...
  public Toogle(audio?): void {
    const tryGet = audio ? this.playList.findIndex((p) => p.src === audio.src) : this.playData.Index;
    if (tryGet < 0) {
  ...
  private PlayIndex(index: number): void {
    ...
    if (index !== -1) {
      ...
      this.playData.MicName =
        this.playList[this.playData.Index].title ?
        this.playList[this.playData.Index].title :
        this.playList[this.playData.Index].name;
